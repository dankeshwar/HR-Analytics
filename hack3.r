...
train_data = train_data %>%
  select(-c(recruitment_channel))

# Convert categorical variables into factors

train_data = train_data %>%
  mutate_at(c("department", "education" ),factor)
...
rf <-randomForest(is_promoted~., data=train_data, ntree=300, na.action=na.exclude, importance=T
                  
) 
print(rf)


#rf2 <-randomForest(is_promoted~., data=val_data, ntree=225, na.action=na.exclude, importance=T
                   
#) 
#print(rf2)


mtry <- tuneRF(train_data[-12], train_data$is_promoted, ntreeTry=400, 
               stepFactor=1.5, improve=0.01, trace=TRUE, plot=TRUE)


rf <-randomForest(is_promoted~., data=train_data, ntree=400, mtry = 6, na.action=na.exclude, importance=T
...
pred3 = predict(rf, test)

print(pred3)

write.csv(pred3, file = "final3.csv")
