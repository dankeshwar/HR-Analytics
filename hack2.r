...
train_data = train_data %>%
  select(-c(employee_id,recruitment_channel))

# Convert categorical variables into factors

train_data = train_data %>%
  mutate_at(c("department", "region", "education" ),factor)
...
rf <-randomForest(is_promoted~., data=train_data, ntree=100, na.action=na.exclude, importance=T
                  
) 
print(rf)


rf2 <-randomForest(is_promoted~., data=val_data, ntree=92, na.action=na.exclude, importance=T
